...
class HayyaBuildShortcode extends HayyaBuildPublic {
    ...
    public function __construct() {
        ...
        foreach (self::getSortCode() as $shortcode) {
    ...
    public static function hayyabuild( $atts ) {
        ...
        $style = "#hb-content-$id { ".HayyaBuildHooks::hb_style($meta)." } \n $csscode \n $style ";
        ...
        if ( ! empty( $class )) $class = ' class="hb-content '.$class.'"';
    ...
    private static function inline_style( $inline_style ) {
        ...
        foreach ( $inline_style as $key => $value ) {
            if ( isset( $value->element ) && ! empty( $value->element ) && isset( $value->style ) && ! empty( $value->style ) ) $style .= "$value->element { $value->style }";
        }
        return $style;
    }
    ...
    private static function render_hayyabuild( $post_content, $id, $class, $style ) {
        $content = $style ? "\n<!-- HayyaBuild Scripts Start -->\n<style class=\"hayyabuild_inline_style\">$style</style>\n<!-- /HayyaBuild Scripts End -->\n" : '';
        $content .= '<div id="hb-container-content-' . $id . '" class="hb-container-content"><div id="hb-content-' . $id . '"'.$class.'>' . $post_content . '</div></div>';
        ...
        if( $recent_posts ) {
            foreach ( $recent_posts as $post ) {
                ...
                if ( has_post_thumbnail($post['ID']) ) {
                    $thumbnail = get_the_post_thumbnail( $post['ID'], 'thumbnail', ['class' => 'hayyabuild-post-thumbnail__news'] );
                    ...
                    $thumbnail = '<img src="' . esc_url( HayyaTheme::hayya_options( 'default-slider-image' ) ) .'" class="attachment-thumbnail" alt="' . esc_attr($post['post_title']) .'" width="150" height="150" />';
                }
